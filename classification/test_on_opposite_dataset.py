...
def get_data_ragtruth(model_name, internal_states_name, val_size=0.15, test_size=0.15, correct_imbalance_train=True, correct_imbalance_val=False, correct_imbalance_test=False, oversampling=False):
    X = []
    y = []
    source_ids = []

    for file_name in os.listdir(INTERNAL_STATES_DIR_RAGTRUTH):
        if file_name.startswith(model_name):
            print(f"Adding data of file {file_name} to data...")
            with open(os.path.join(INTERNAL_STATES_DIR_RAGTRUTH, file_name), 'rb') as handle:
                file_json = pickle.load(handle)
                for passage_data in file_json:
                    for sentence_data in passage_data["sentence_data"]:
