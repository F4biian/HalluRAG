...
def classify(art: dict, section_before_passage: str, passage_text: str, question: str, anwer_quote: str, llm_output: str):
    title = art["title"]

    system_msg = "You are given a QUESTION which is solely based on the SENTENCE. You are perfect at comparing the OUTPUT with the REFERENCE ANSWER. The REFERENCE ANSWER is the correct answer to the QUESTION. The OUTPUT is another answer that you check for mistakes."
    prompt = (
        "### PREVIOUS CONTEXT\n"
        f"Title: '{title}'\n"
        ...
        "\n"
        "### OBJECTIVE\n"
        f"Compare the REFERENCE ANSWER with the OUTPUT by looking for mistakes or facts that are not part of the REFERENCE ANSWER. You also classify the type of mistake:\n"
        "- INCORRECT: OUTPUT is not entirely correct, contains mistake\n"
        "- NO_HELP: OUTPUT does not answer the QUESTION, OUTPUT does not refer to the SENTENCE\n"
        "After sharing your examination thoughts, for each mistake you always quote the \"wrong\" part from the OUTPUT and the \"correct\" part from the REFERENCE ANSWER. If there are no mistakes, the \"mistakes\" list is empty.\n"
        "\n"
        "### RESPONSE\n"
        ...
        "      \"sentence_quote\": <correct words quoted from the SENTENCE> # ignore \"sentence_quote\" if type is NO_HELP\n"
        "    }},\n"
        "    ...\n"
        "  ]\n"
        "}}\n"
